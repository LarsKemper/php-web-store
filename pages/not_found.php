<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>NOT FOUND</title>
    <link rel="stylesheet" href="./../style/app.css">

</head>

<body>
<main aria-labelledby="pageTitle" class="flex items-center justify-center h-screen bg-gray-100">
    <div class="p-4 space-y-4">
        <div class="flex flex-col items-start space-y-3 sm:flex-row sm:space-y-0 sm:items-center sm:space-x-3">
            <p class="font-semibold text-secondary-default text-9xl">404</p>
            <div class="space-y-2">
                <h1 id="pageTitle" class="flex items-center space-x-2">
                    <svg aria-hidden="true" class="w-6 h-6 text-secondary-default"
                         xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                              d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                    </svg>
                    <span class="text-xl font-medium text-gray-600 sm:text-2xl">
                            Oops! Page not found.
                        </span>
                </h1>
                <p class="text-base font-normal text-gray-600">
                    The page you are looking for was not found.
                </p>
                <p class="text-base font-normal text-gray-600">
                    You may return to
                    <a href="/php-web-store/index.php" class="text-blue-600 hover:underline">home
                        page</a> or try
                    using the search form.
                </p>
            </div>
        </div>
    </div>
</main>
</body>

</html>